 ...
 */
public class TopFiveExperienceTypesForUser implements Filter {

    public static final int TOP_FIVE_EXPERIENCE_TYPE_LIMIT = 5;
    private DatabaseHelper dbHelper;
    private User user;

    public TopFiveExperienceTypesForUser(DatabaseHelper dbHelper, User user){
        this.dbHelper = dbHelper;
        this.user = user;
    }

    @Override
    public List<GoogleResultMock> filter(List<GoogleResultMock> suggestionsToFilter) {
        final Set<ExperienceType> experienceTypes = getSetOfExperienceTypes(suggestionsToFilter);
        ...
        for(final GoogleResultMock result: suggestionsToFilter){
